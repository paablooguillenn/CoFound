...
import { backfillSeedExtras, getStats, listReports, runSeedScript } from '../services/admin.service';
...
export const reseedController = async (_request: Request, response: Response, next: NextFunction) => {
  try {
    const summary = await runSeedScript();
    response.json({ ok: true, summary });
  } catch (error) {
    next(error);
  }
